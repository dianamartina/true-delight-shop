                    ...
                    ? <div className="">
                            ...
                            <div className="row">
                        {
                            ...
                            products.map(product => {
                                ...
                                return <div className="col-lg-3 col-md-4 col-sm-6 col-6 d-flex justify-content-center align-items-stretch font-italic" key={product.id}>
                                    <div className="gold-border pb-0 d-flex flex-column position-relative">
                                        <img src={product.image} alt="Produs" className="w-100"/>
                                        <h3 className="mt-3 wishlist-name">{ product.name }</h3>
                                        <p >{ product.price }<span className="ml-1 text-14">{ product.currency }</span></p>

                                        <div className=" flex-grow-1"></div>
                                        
                                        <button 
                                            onClick={() => addToCartInStore(product)}
                                            className="btn btn-gold btn-sm mb-4 ">Add to cart
                                        </button>

                                        <span onClick={()=>removeProduct(product)} className="delete p-3 wishlist-delete" ><DeleteCercle /></span>
                                    </div>
                                </div>
                            })
                        }
